...
static EC_T_PROCESS_VAR_INFO s_OutputProcessVar_1;
static EC_T_PROCESS_VAR_INFO s_OutputProcessVar_2;
static EC_T_PROCESS_VAR_INFO s_OutputProcessVar_3;
static EC_T_PROCESS_VAR_INFO s_OutputProcessVar_4;
static EC_T_PROCESS_VAR_INFO s_OutputProcessVar_5;
static EC_T_PROCESS_VAR_INFO s_OutputProcessVar_6;
static EC_T_PROCESS_VAR_INFO s_OutputProcessVar_7;
static EC_T_PROCESS_VAR_INFO s_OutputProcessVar_8;

static EC_T_BYTE bOutputCur_1;
static EC_T_BYTE bOutputCur_2;
static EC_T_BYTE bOutputCur_3;
static EC_T_BYTE bOutputCur_4;
static EC_T_BYTE bOutputCur_5;
static EC_T_BYTE bOutputCur_6;
static EC_T_BYTE bOutputCur_7;
static EC_T_BYTE bOutputCur_8;

static uint64_t time_now_us = 0;
...
static EC_T_DWORD myAppPrepare(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
	...
	if (dwRes != EC_E_NOERROR)
	{
		printf("ERROR: s_OutputProcessVar_1 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
	else
	{
		printf("INFO: s_OutputProcessVar_1 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}

	dwRes = ecatFindOutpVarByName("SubDevice_1003 [EL2008].Channel 2.Output", &s_OutputProcessVar_2);
	if (dwRes != EC_E_NOERROR)
	{
		printf("ERROR: s_OutputProcessVar_2 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
	else
	{
		printf("INFO: s_OutputProcessVar_2 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}

	dwRes = ecatFindOutpVarByName("SubDevice_1003 [EL2008].Channel 3.Output", &s_OutputProcessVar_3);
	if (dwRes != EC_E_NOERROR)
	{
		printf("ERROR: s_OutputProcessVar_3 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
	else
	{
		printf("INFO: s_OutputProcessVar_3 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}

	dwRes = ecatFindOutpVarByName("SubDevice_1003 [EL2008].Channel 4.Output", &s_OutputProcessVar_4);
	if (dwRes != EC_E_NOERROR)
	{
		printf("ERROR: s_OutputProcessVar_4 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
	else
	{
		printf("INFO: s_OutputProcessVar_4 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}

	dwRes = ecatFindOutpVarByName("SubDevice_1003 [EL2008].Channel 5.Output", &s_OutputProcessVar_5);
	if (dwRes != EC_E_NOERROR)
	{
		printf("ERROR: s_OutputProcessVar_5 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
	else
	{
		printf("INFO: s_OutputProcessVar_5 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}

	dwRes = ecatFindOutpVarByName("SubDevice_1003 [EL2008].Channel 6.Output", &s_OutputProcessVar_6);
	if (dwRes != EC_E_NOERROR)
	{
		printf("ERROR: s_OutputProcessVar_6 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
	else
	{
		printf("INFO: s_OutputProcessVar_6 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}

	dwRes = ecatFindOutpVarByName("SubDevice_1003 [EL2008].Channel 7.Output", &s_OutputProcessVar_7);
	if (dwRes != EC_E_NOERROR)
	{
		printf("ERROR: s_OutputProcessVar_7 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
	else
	{
		printf("INFO: s_OutputProcessVar_7 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}

	dwRes = ecatFindOutpVarByName("SubDevice_1003 [EL2008].Channel 8.Output", &s_OutputProcessVar_8);
	if (dwRes != EC_E_NOERROR)
	{
		printf("ERROR: s_OutputProcessVar_8 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
	else
	{
		printf("INFO: s_OutputProcessVar_8 ecatFindOutpVarByName returns with %s (0x%x)\n", ecatGetText(dwRes), dwRes);
	}
...
//    std::cout << "time_now_us: " << time_now_us << ", time_now_s" << time_now_us/1000000 << std::endl;

    uint64_t T_tota_us = 500000;
    bool led_on = (time_now_us % T_tota_us) > (T_tota_us/2);
	...
	EC_SETBITS(pbyPdOut, reinterpret_cast<EC_T_BYTE*>(&bOutputCur_1), s_OutputProcessVar_1.nBitOffs, s_OutputProcessVar_1.nBitSize); // frame output, rx pdo
	EC_SETBITS(pbyPdOut, reinterpret_cast<EC_T_BYTE*>(&bOutputCur_2), s_OutputProcessVar_2.nBitOffs, s_OutputProcessVar_2.nBitSize); // frame output, rx pdo
	EC_SETBITS(pbyPdOut, reinterpret_cast<EC_T_BYTE*>(&bOutputCur_3), s_OutputProcessVar_3.nBitOffs, s_OutputProcessVar_3.nBitSize); // frame output, rx pdo
	EC_SETBITS(pbyPdOut, reinterpret_cast<EC_T_BYTE*>(&bOutputCur_4), s_OutputProcessVar_4.nBitOffs, s_OutputProcessVar_4.nBitSize); // frame output, rx pdo
	EC_SETBITS(pbyPdOut, reinterpret_cast<EC_T_BYTE*>(&bOutputCur_5), s_OutputProcessVar_5.nBitOffs, s_OutputProcessVar_5.nBitSize); // frame output, rx pdo
	EC_SETBITS(pbyPdOut, reinterpret_cast<EC_T_BYTE*>(&bOutputCur_6), s_OutputProcessVar_6.nBitOffs, s_OutputProcessVar_6.nBitSize); // frame output, rx pdo
	EC_SETBITS(pbyPdOut, reinterpret_cast<EC_T_BYTE*>(&bOutputCur_7), s_OutputProcessVar_7.nBitOffs, s_OutputProcessVar_7.nBitSize); // frame output, rx pdo
	EC_SETBITS(pbyPdOut, reinterpret_cast<EC_T_BYTE*>(&bOutputCur_8), s_OutputProcessVar_8.nBitOffs, s_OutputProcessVar_8.nBitSize); // frame output, rx pdo
