...
main(int argc, char *argv[]) {
	...
	channel = ChannelCreate_r(_NTO_CHF_COID_DISCONNECT);
	if(channel < 0) {
	...
	for( ;; ) {
		...
		case _PULSE_CODE_COIDDEATH: ;
			// Somebody else's channel has been destroyed.
			unsigned missing_list = hyp_shm_verify(hsp, ~0u);
			printf("missing list = %x\n", missing_list);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hypervisor/branches/qh_2.1/apps/hypervisor/demos/shmem-host/hhstest.c $ $Rev: 989188 $")
#endif
