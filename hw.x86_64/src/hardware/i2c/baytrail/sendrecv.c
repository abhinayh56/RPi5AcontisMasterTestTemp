...
#define _i2c_slogf(dev, level, fmt, ...) \
    (i2c_slogf((dev)->verbosity, (level), (fmt), ##__VA_ARGS__))

static i2c_status_t baytrail_i2c_sendrecv(void* const hdl, void* const txbuf,
                const uint32_t txlen, void* const rxbuf, const uint32_t rxlen, const uint32_t stop)
{
    baytrail_dev_t  *dev = hdl;
    ...
    /* Active I2C bus */
    if (baytrail_i2c_bus_active(dev) != 0) {
    ...
    ret = baytrail_i2c_wait_complete(dev);
...
int _i2c_master_sendrecv(resmgr_context_t *ctp, io_devctl_t *msg, i2c_ocb_t *ocb)
{
    ...
    if (msg->i.nbytes < sizeof(*hdr)) {
        _i2c_slogf(dev, _SLOG_ERROR, "Incomplete sendrecv hdr");
        return EINVAL;
    }

    hdr = _DEVCTL_DATA(msg->i);
    ...
    if ((sizeof(*hdr) + hdr->send_len) > msg->i.nbytes) {
        _i2c_slogf(dev, _SLOG_ERROR, "Incomplete send data");
        return EINVAL;
    }

    if ((sizeof(*hdr) + hdr->recv_len) > msg->i.nbytes) {
        _i2c_slogf(dev, _SLOG_ERROR, "Receive buffer too short");
        return EINVAL;
    }
    ...
    if ((sizeof(*msg) + sizeof(*hdr) + hdr->send_len) > ctp->msg_max_size) {
        int status;
        status = (int)resmgr_msgread(ctp, dev->buf, (size_t)hdr->send_len, sizeof(*msg) + sizeof(*hdr));
        if (status < 0) {
            return errno;
        }
        if ((unsigned)status < hdr->send_len) {
            return EFAULT;
        }
        txbuf = dev->buf;
    } else {
    ...
    if (dev->bus_speed != ocb->bus_speed) {
        if (-1 == dev->mfuncs.set_bus_speed(dev->hdl, ocb->bus_speed, NULL)) {
            _i2c_slogf(dev, _SLOG_ERROR, "Bad bus speed: %d: %s", ocb->bus_speed, strerror(errno));
            return EIO;
        }
    ...
    if (-1 == dev->mfuncs.set_slave_addr(dev->hdl, hdr->slave.addr, hdr->slave.fmt)) {
        _i2c_slogf(dev, _SLOG_ERROR, "Bad slave address: %x (%s): %s", hdr->slave.addr,
                   (hdr->slave.fmt == I2C_ADDRFMT_7BIT)? "7-bit": "10-bit", strerror(errno));
        return EIO;
    }
    ocb->status = 0;

    if (hdr->send_len + hdr->recv_len > 0) {
        mstatus = baytrail_i2c_sendrecv(dev->hdl, txbuf, hdr->send_len, rxbuf, hdr->recv_len, hdr->stop);
        ocb->status = mstatus;
        if (mstatus != I2C_STATUS_DONE) {
            if (!(mstatus & I2C_STATUS_DONE)) {
                _i2c_slogf(dev, _SLOG_ERROR, "Master sendrecv did not terminate for slave 0x%x", hdr->slave.addr);
            }
            if (mstatus & I2C_STATUS_ERROR) {
                _i2c_slogf(dev, _SLOG_ERROR, "Master sendrecv timeout for slave 0x%x", hdr->slave.addr);
            }
            if (mstatus & I2C_STATUS_ADDR_NACK) {
                _i2c_slogf(dev, _SLOG_WARNING, "Master sendrecv address NACK for slave 0x%x", hdr->slave.addr);
            }
            if (mstatus & I2C_STATUS_DATA_NACK) {
                _i2c_slogf(dev, _SLOG_WARNING, "Master sendrecv data NACK for slave 0x%x", hdr->slave.addr);
            }
            if (mstatus & I2C_STATUS_ARBL) {
                _i2c_slogf(dev, _SLOG_WARNING, "Master sendrecv ARBL for slave 0x%x", hdr->slave.addr);
            }
            if (mstatus & I2C_STATUS_BUSY) {
                _i2c_slogf(dev, _SLOG_ERROR, "Master sendrecv BUSY for slave 0x%x", hdr->slave.addr);
            }
            if (mstatus & I2C_STATUS_ABORT) {
                _i2c_slogf(dev, _SLOG_ERROR, "Master sendrecv ABORT for slave 0x%x", hdr->slave.addr);
            }
            /* General error code - more info is available through a devctl */
            return EIO;
        }
        dev->hdr.flags |= IOFUNC_ATTR_MTIME | IOFUNC_ATTR_CTIME;
    }

    memset(&msg->o, 0, sizeof(msg->o));
    msg->o.ret_val = (int)hdr->recv_len;
    msg->o.nbytes = (unsigned)sizeof(*hdr) + hdr->recv_len;
