...
/*
 * AMD IOAPIC specific configuration
 *
 * Specifically this implementation accommodates AMD's use of 8 IOAPIC ID bits
 *
*/
 ...
 *
 * AMD specific IOAPIC configuration. If <ioapic_cfg> is not NULL, it must point
 * to a <num> element array of 'ioapic_cfg_t' entries. There does not need to be
 * a 1:1 correspondence between the number of IOAPIC entries and <num>. That is,
 * the caller can specify only values for those interrupt entries to be modified
 * (as specified by 'ioapic_cfg[i].intsrc.dest_apic_intin') however the caller
 * must also ensure that the unspecified entries do not conflict with any entries
 * being modified.
 *
...
static unsigned configure(const unsigned ioapic_id, const ioapic_cfg_t *ioapic_cfg, const int num)
{
	...
	{
		kprintf("Expected AMD IOAPIC %u not found\n", ioapic_id);
		return 0;
	}
...
static ioapic_accessfnc_t _ioapic_accessfncs_amd =
{
...
ioapic_accessfnc_t *ioapic_accessfncs_amd = &_ioapic_accessfncs_amd;
...
/* get/set the IOAPIC ID preserving all other bits
 *
 * The IO-APIC ID is 8 bits (bit[31:24]) instead of the specification defined
 * 4 bits on AMD (Ryzen) so we'll use 8 bits for all AMD going forward. Since
 * family 0x17 and newer (ie. Ryzen) will be the most prevalent AMD platform
 * going forward, and because this does still work fine on older AMD, we'll not
 * bother being family selective. If we need to, that change can be easily
 * accommodated
 */
static unsigned ioapic_get_id(const PADDR_T ioapic_paddr)
{
	return (ioapic_read(ioapic_paddr, ioapic_reg_ID) & 0xFF000000) >> 24;
}

static void ioapic_set_id(const PADDR_T ioapic_paddr, unsigned ioapic_id)
{
	const uint32_t id_reg = ioapic_read(ioapic_paddr, ioapic_reg_ID) & ~0xFF000000;
	ioapic_write(ioapic_paddr, ioapic_reg_ID, ((ioapic_id & 0xFF) << 24) | id_reg);
