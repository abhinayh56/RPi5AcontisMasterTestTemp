 ...
 * after reset
 *
 * The following device/function/offsets exist in the Tunnel Creek
 * NOTE: that this does not include any devices external to the Tunnel Creek
 * 		 processor such as those found in the Topcliff IOH
 *
 * D23:F0 FD register offset 0xFC	(PCIe port 0)
 * D24:F0 FD register offset 0xFC	(PCIe port 1)
 * D25:F0 FD register offset 0xFC	(PCIe port 2)
 * D26:F0 FD register offset 0xFC	(PCIe port 3)
 * D27:F0 FD register offset 0xFC	(HD Audio)
 * D2:F0 FD register offset 0xC4	(Integrated Graphics)
 * D3:F0 FD register offset 0xC4	(SVDO Display)
 *
...
unsigned tnc_function_disable(unsigned device, unsigned function)
{
	unsigned fd_offset;
	unsigned disabled;
	...
	switch(device)
	{
		...
		case 23:	// devices 23 thru 27 have only function 0
		case 24:
		case 25:
		case 26:
		case 27:
	...
	/* need to also clear the command register for the device before it is disabled */
	...
	/* all reads at this point will actually return F's but the test is still valid */
