...
// Intel IOH IOAPIC specific configuration
...
typedef enum
{
	ioh_ioapic_reg_ID			= 0x00,
	ioh_ioapic_reg_VERSION		= 0x01,
	ioh_ioapic_reg_REDIR_TBL_0	= 0x10,
	
} ioh_ioapic_reg_index_e;

#define ioh_ioapic_reg_REDIR_TBL(n)	(ioh_ioapic_reg_REDIR_TBL_0 + (2 * (n)))
...
//		kprintf("[%u]: %L   ", i, redir_val);
		kprintf("[%u]: %x %x   ", i, ioapic_read(ioapic_paddr, ioh_ioapic_reg_REDIR_TBL(i)+1),  ioapic_read(ioapic_paddr, ioh_ioapic_reg_REDIR_TBL(i)));
	}
	kprintf("\n");
}
 ...
 * configure
 * 
 * IOH Southbridge specific IOAPIC configuration including SCH (poulsbo) and
 * Topcliff.
 * If <ioapic_cfg> is not NULL, it must point to a <num> element array of
 * 'ioapic_cfg_t' entries. There does not need to be a 1:1 correspondence between
 * the number of IOAPIC entries and <num>. That is, the caller can specify only
 * values for those interrupt entries to be modified (as specified by
 * 'ioapic_cfg[i].intsrc.dest_apic_intin') however the caller must also ensure
 * that the unspecified entries do not conflict with any entries being modified.
 * 
...
static unsigned configure(const unsigned ioapic_id, const ioapic_cfg_t *ioapic_cfg, const int num)
{
	...
	{
		kprintf("Expected IOH IOAPIC %u not found\n", ioapic_id);
		return 0;
	}

	/* IOH IOAPIC address is fixed and it does not need to be explicitly enabled */
	if (ioapic_paddr != 0xFEC00000) return 0;
	...
	{
		kprintf("Only %u IOH interrupts available, not %u\n", ioapic_num_entries(ioapic_paddr), num);
		return 0;
	}
	...
	for(i=0; i<num; i++)
	{
		...
		{
			/* this table maps ioapic_cfg[i].intsrc.int_type types to IOH interrupt types */
...
static ioapic_accessfnc_t _ioapic_accessfncs_ioh =
{
...
ioapic_accessfnc_t *ioapic_accessfncs_ioh = &_ioapic_accessfncs_ioh;
...
static unsigned ioapic_num_entries(const PADDR_T ioapic_paddr)
{
	...
	return ((ioapic_read(ioapic_paddr, ioh_ioapic_reg_VERSION) & 0x00FF0000) >> 16) + 1;
}

static unsigned ioapic_get_id(const PADDR_T ioapic_paddr)
{
	return (ioapic_read(ioapic_paddr, ioh_ioapic_reg_ID) & 0x0F000000) >> 24;
}

static void ioapic_set_id(const PADDR_T ioapic_paddr, unsigned ioapic_id)
{
	/* set the id preserving all other bits */
	uint32_t id_reg = ioapic_read(ioapic_paddr, ioh_ioapic_reg_ID) & ~0x0F000000;
	ioapic_write(ioapic_paddr, ioh_ioapic_reg_ID, ((ioapic_id & 0xF) << 24) | id_reg);
...
{
	return ioapic_read(ioapic_paddr, ioh_ioapic_reg_VERSION) & 0x000000FF;
}
#endif	/* APIC_DEBUG */
...
static uint64_t ioapic_read_entry(const PADDR_T ioapic_paddr, unsigned entry_num)
{
	...
	const unsigned reg_num = ioh_ioapic_reg_REDIR_TBL(entry_num);
	uint64_t data;
